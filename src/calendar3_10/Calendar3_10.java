...
//프롬프트를 출력한다.
public class Calendar3_10 {
	static int[] MONTH_LAST_DAY = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
	static int[] MONTH_LEAP_DAY = {31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
...
//			}
	...
	public static int  getMonthMaxDay(int m) {
		...
		int nYear = cal.get(Calendar.YEAR);		//년도 구하기

		//윤년 : 4로 나누어 떨어진다. 
		//100으로 나누어 떨어지는건 평년이다. 그중에 400으로 나누어 떨어지는건 윤년이다.
		if(nYear%4 == 0 && (nYear%100 != 0 && nYear%400 == 0)) {
			return MONTH_LEAP_DAY[m-1];	//윤년
		}else {
			return MONTH_LAST_DAY[m-1];	//평년
		}
	}
